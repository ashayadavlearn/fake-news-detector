...
def too_large(e):
    if request.path.startswith("/api/"):
        return jsonify({"ok": False, "errors": ["That submission is too large. Please shorten it."]}), 413
    flash("That submission was too large. Please shorten it and try again.", "danger")
    return redirect(request.referrer or url_for("index"))


@app.errorhandler(CSRFError)
def handle_csrf_error(e):
    # A missing/expired CSRF token (e.g. a form left open too long) previously
    # fell through to Flask's raw, unstyled default error page -- and for the
    # AJAX detector endpoint it returned HTML where the JS expected JSON,
    # causing a misleading "Network error" alert. Handle both cases explicitly.
    if request.path.startswith("/api/"):
        return jsonify({"ok": False, "errors": ["Your session expired. Please refresh the page and try again."]}), 400
